...
public class DoubleLinkedList {
    ...
    public void remove(int index) throws Exception {
        if (isEmpty() || index >= size) {120 throw new Exception("Nilai indeks di luar batas");
        } else if (index == 0) {
            removeFirst();
        } else {
            Node current = head;
    ...
    public void searchData(int caridata) {
        int i = 1;
        boolean a = false;
        Node current = head;
        if (head == null) {
        ...
        if (a) {
            System.out.println("Data " + caridata + " berada di node ke-" + i);
        } else {
